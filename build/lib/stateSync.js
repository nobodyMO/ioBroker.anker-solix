    ...
    for (const entityId of entityIds) {
      ...
      if ((meta == null ? void 0 : meta.kind) === "list") {
        if (entityId === "max_total_ac_output" && ((_c = device.max_total_ac_output_options) == null ? void 0 : _c.length)) {
          ...
          common.states = import_entities.PPS_AC_OUTPUT_MODE_STATES;
        } else if (entityId === "pps_dc_12v_output_mode") {
          common.states = import_entities.PPS_DC_12V_OUTPUT_MODE_STATES;
        } else if (entityId === "pps_device_timeout_minutes") {
          common.states = import_entities.PPS_DEVICE_TIMEOUT_MINUTES_STATES;
        } else if (entityId === "pps_display_timeout_seconds") {
          common.states = import_entities.PPS_DISPLAY_TIMEOUT_SECONDS_STATES;
        } else if (entityId === "pps_display_mode") {
          common.states = import_entities.PPS_DISPLAY_MODE_STATES;
        } else if (entityId === "pps_temp_unit_fahrenheit_mode") {
          common.states = import_entities.PPS_TEMP_UNIT_FAHRENHEIT_STATES;
        } else if (entityId === "pps_usb_status") {
          common.states = import_entities.PPS_USB_STATUS_STATES;
