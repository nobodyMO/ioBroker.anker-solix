...
const authCacheBackup_1 = require("./lib/authCacheBackup");
const configHelpers_1 = require("./lib/configHelpers");
const controlQueue_1 = require("./lib/controlQueue");
const curtailmentPower_1 = require("./lib/curtailmentPower");
const curtailmentRunner_1 = require("./lib/curtailmentRunner");
const curtailmentStates_1 = require("./lib/curtailmentStates");
const ensurePython_1 = require("./lib/ensurePython");
const pythonBridge_1 = require("./lib/pythonBridge");
const services_1 = require("./lib/services");
const curtailmentConfig_1 = require("./lib/curtailmentConfig");
const systemBatPower_1 = require("./lib/systemBatPower");
const stateSync_1 = require("./lib/stateSync");
class AnkerSolix extends utils.Adapter {
    pollTimer;
    controlQueue = new controlQueue_1.ControlQueue();
    ...
    getAuthCachePaths() {
        return (0, authCacheBackup_1.resolveAuthCachePaths)(utils.getAbsoluteInstanceDataDir(this), (this.config.username || "").trim());
    }
    getAuthCacheFile() {
        return this.getAuthCachePaths().cacheFile;
    }
    ensureAuthCacheBackedUp() {
        (0, authCacheBackup_1.backupAuthCacheOnce)(this.getAuthCachePaths(), this.log);
    }
    logAuthCacheStatus() {
    ...
    getBridgeConfig() {
        const cacheDir = this.getAuthCacheDir();
        const selectedIds = (0, configHelpers_1.parseSelectedDeviceIds)(this.config.selectedDeviceIds);
        return {
    ...
    async ensurePythonDeps(force = false) {
        ...
        const result = await (0, ensurePython_1.runPythonInstaller)(this.config.pythonPath || "", this.log);
    ...
    async pollOnceBody() {
        ...
        try {
            const result = await (0, pythonBridge_1.runBridge)("poll", this.getBridgeConfig(), this.config.pythonPath || "", this.log);
            ...
            if (pollDevices?.length) {
                ...
                this.siteSolarbanks = (0, systemBatPower_1.buildSiteSolarbankMap)(pollDevices);
                await (0, stateSync_1.syncDevices)(this, pollDevices);
                if (this.batPowerAggregationEnabled()) {
                    await (0, systemBatPower_1.refreshAllSystemBatPowerSums)(this, this.siteSolarbanks);
                }
            }
            if (result.nickname) {
    ...
    getPrimaryDeviceId() {
        const selected = (0, configHelpers_1.parseSelectedDeviceIds)(this.config.selectedDeviceIds);
        if (selected[0]) {
            return selected[0];
        }
    ...
    async handleServiceTrigger(stateId) {
        ...
        try {
            ...
            const result = await (0, pythonBridge_1.runBridge)("service", serviceConfig, this.config.pythonPath || "", this.log);
            if (action === "get_schedule" && result.schedule !== undefined) {
                await this.setState(services_1.SERVICE_STATES.scheduleJson, JSON.stringify(result.schedule, null, 2), true);
            }
            if (action === "export_systems" && result.path) {
                await this.setState(services_1.SERVICE_STATES.exportResult, String(result.path), true);
            }
            if (action === "get_system_info" && result.system !== undefined) {
                await this.setState(services_1.SERVICE_STATES.systemInfo, JSON.stringify(result.system, null, 2), true);
            }
            await this.setState(stateId, false, true);
        }
        catch (error) {
    ...
    collectSiteSolarbankSocs(siteId) {
        ...
        for (const [deviceId, ctx] of this.deviceContexts) {
            ...
            const soc = (0, curtailmentPower_1.normalizeSocPercent)(entities?.state_of_charge ?? entities?.battery_soc);
            if (soc === undefined) {
                continue;
            }
    ...
    async applyAdapterControl(deviceId, control, value, deviceContext, opts) {
        await (0, pythonBridge_1.runBridge)("set", {
            ...this.getBridgeConfig(),
            deviceId,
            control,
            value,
    ...
    getCurtailmentConfig() {
        ...
        return {
            ...
            modeAfter,
            minPvW: (0, curtailmentPower_1.normalizeMinPvForCurtailmentW)(this.config.curtailmentMinPvW),
    ...
    refreshCurtailmentDeviceIds() {
        ...
        for (const d of (0, curtailmentConfig_1.resolveCurtailmentDevices)(this.getCurtailmentConfig())) {
            if (d.enabled) {
    ...
    async runCurtailmentExportOnPvChange(deviceId, livePvW) {
        ...
        try {
            await (0, curtailmentRunner_1.runCurtailmentOnPvChange)(this.getCurtailmentHost(), this.getCurtailmentConfig(), deviceId, livePvW);
        }
        catch (err) {
    ...
    async handleSolarbankBatPowerChanged(deviceSn) {
        ...
        await (0, systemBatPower_1.sumSolarbankBatPowerToSystem)(this, ctx.site_id, sns);
    }
    ...
    async onStateChange(id, state) {
        ...
        if (this.batPowerAggregationEnabled()) {
            const bat = (0, systemBatPower_1.parseSolarbankBatPowerStateId)(this.namespace, id);
            if (bat) {
        ...
        const control = (0, stateSync_1.parseControlStateId)(this.namespace, id);
        if (!control) {
            return;
        }
    ...
    async onMessage(obj) {
        ...
        const formatAuthCacheStatusLine = (paths) => {
            const status = (0, authCacheBackup_1.authCacheStatus)(paths);
            if (!paths.email) {
    ...
    async onReady() {
        ...
        await (0, services_1.setupServiceStates)(this);
        await (0, curtailmentStates_1.setupCurtailmentStates)(this);
    ...
    onUnload(callback) {
        ...
        void (0, pythonBridge_1.stopBridgeDaemon)().finally(() => callback());
    }
}
