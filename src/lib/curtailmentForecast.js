...
export function normalizeForecastPowerW(raw, unit) {
...
export async function readHourlyForecast(basePath, getState, getObject) {
...
export function detectCurtailmentWindow(forecast, acLimitW) {
    const overHours = [];
...
export function currentPhase(window, nowHour) {
    if (!window.today) {
        return "idle";
    }
...
export function remainingCurtailmentHours(window, nowHour) {
...
export function forecastPowerAtHour(forecast, hour) {
 ...
 */
export function forecastExportTargetW(forecast, nowHour, window) {
    if (!window.today) {
        return 0;
    }
