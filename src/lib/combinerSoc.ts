...
export interface SolarbankSocSample {
	socPercent: number;
	capacityWh?: number;
}

/** Weighted mean when all banks have capacity; otherwise arithmetic mean. */
export function aggregateSolarbankSoc(banks: SolarbankSocSample[]): number | undefined {
	if (!banks.length) {
		return undefined;
	}
	if (banks.length === 1) {
...
export function normalizeSocPercent(raw: unknown): number | undefined {
