...
export const PV_SENSOR_IDS = ["total_pv_power", "input_power", "solar_power_total"];
export function normalizeMinPvForCurtailmentW(raw) {
    const n = Number(raw);
...
export function hasSolarGenerationForCurtailment(livePvW, minPvW) {
...
export function systemTotalPvStatePath(namespace, siteId) {
    return `${namespace}.system.${siteId}.sensors.total_pv_power`;
}
export function pvSensorStatePaths(namespace, deviceId) {
    const paths = [];
...
export function parseSystemPvStateId(namespace, stateId) {
...
export function parsePvSensorStateId(namespace, stateId) {
    const prefix = `${namespace}.`;
...
export function isPvSensorEntity(entityId) {
    return PV_SENSOR_IDS.includes(entityId);
}
export function isPvGenerationSensor(entityId) {
...
export function readPvFromEntities(entities) {
    if (!entities) {
        return 0;
    }
    let max = 0;
...
export async function readLivePvPowerW(host, deviceId) {
...
export function resolveBeforeExportW(livePvW) {
...
export function calcMissingChargeWh(batteryCapacityWh, socPercent) {
...
export function calcMaxChargeW(missingWh, hoursRemaining) {
...
export async function readSocPercentForCurtailment(host, deviceId) {
    const fromEntities = host.getDeviceEntities?.(deviceId);
    if (fromEntities) {
        const total = normalizeSocPercent(fromEntities.total_state_of_charge ?? fromEntities.computed_total_soc ?? fromEntities.total_soc);
        if (total !== undefined) {
...
export function resolveActiveExportW(livePvW, maxChargeW) {
    if (livePvW <= 0) {
        return 0;
    }
    return Math.max(0, Math.round(livePvW - Math.max(0, maxChargeW)));
}
export function resolveCurtailmentSetpoints(phase, livePvW, maxChargeW, _forecast, _nowHour, _window) {
    if (phase === "before") {
